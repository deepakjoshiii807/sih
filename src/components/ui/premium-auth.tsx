...
export type ProfileType = "student" | "industry" | "academician" | "institute-admin";
...
export const PROFILE_OPTIONS: ProfileOption[] = [
  {
    id: "student",
    label: "Student",
    hint: "Find courses, scholarships & internships",
    icon: GraduationCap,
  },
  {
    id: "industry",
    label: "Industry",
    hint: "Post jobs & hire young talent",
    icon: Building2,
  },
  {
    id: "academician",
    label: "Academician",
    hint: "Publish courses & guide learners",
    icon: BookOpen,
  },
  {
    id: "institute-admin",
    label: "Institute Admin",
    hint: "Manage your institution's presence",
    icon: School,
  },
];
...
interface AuthFormProps {
  ...
  onSuccess?: (userData: { email: string; name?: string; profile?: ProfileType }) => void;
...
// Profile Type Selector — 4 account types shown on signup
  ...
  return (
    <div className="space-y-2">
      <p className="text-sm font-medium text-foreground">
        Choose your account type <span className="text-destructive">*</span>
      </p>
            ...
            <button
              ...
              className={cn(
                ...
                isSelected
                  ...
                  : "border-input bg-muted/40 hover:border-primary/40 hover:bg-muted/70"
              )}
            >
              <span
                className={cn(
 ...
 *
 * Supports login, signup (with 4 account types: Student / Industry /
 * Academician / Institute Admin) and password reset flows with real-time
 * validation and password strength indicators.
 */
export function AuthForm({
  ...
  const selectedProfile = PROFILE_OPTIONS.find((p) => p.id === formData.profile);
  ...
  const validateField = useCallback(
    (field: keyof FormData, value: string | boolean | ProfileType | null) => {
      let error = "";
  ...
  const validateForm = useCallback(() => {
    ...
    if (authMode === "signup") {
      fieldsToValidate.push("name", "confirmPassword", "agreeToTerms", "profile");
    }
  ...
  // Handle form submission - replace with actual authentication logic
      ...
      return (
        <div className="space-y-4 animate-in fade-in-50 slide-in-from-right-5">
          <div className="text-center mb-6">
            ...
            {selectedProfile && (
              <p className="mt-2 inline-flex items-center gap-1.5 rounded-full bg-primary/10 border border-primary/30 px-3 py-1 text-xs text-primary">
                <selectedProfile.icon className="h-3.5 w-3.5" />
                {selectedProfile.label} account
              </p>
            )}
          </div>
        ...
        <div className="text-center space-y-6 animate-in fade-in-50 slide-in-from-right-5">
          ...
          <div>
            ...
            <p className="text-muted-foreground">
              {selectedProfile
                ? `Your ${selectedProfile.label.toLowerCase()} account has been created successfully.`
      ...
      <div className="space-y-4 animate-in fade-in-50 slide-in-from-right-5">
        {authMode === "signup" && (
          <ProfileSelector
            ...
            onSelect={(profile) => handleInputChange("profile", profile)}
